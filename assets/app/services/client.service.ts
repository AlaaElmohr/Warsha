...
changePassword(password){
  ...
  return this.http.patch('http://app-warsha-1.herokuapp.com/client/changepassword/' + id, body)
      .map((response: Response) => response.json())
      .catch((error: Response) => Observable.throw(error.json()));
}
getClients(){
  return this.http.get('http://app-warsha-1.herokuapp.com/client')
...
getClient(id){
  const token = localStorage.getItem('token')
      ...
      : '';
let result;
  result= this.http.get('http://app-warsha-1.herokuapp.com/client/'+id+token);
