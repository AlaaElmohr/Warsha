...
export class ContractService{
    ...
    addContract(contract){
      ...
      const token = localStorage.getItem('token')
          ...
          : '';
      return this.http.post('https://warsha-2.herokuapp.com/contract' + token, body, {headers: headers})
    ...
    finishContract(contract){
      ...
      const token = localStorage.getItem('token')
          ? '?token=' + localStorage.getItem('token')
          : '';
      return this.http.patch('https://warsha-2.herokuapp.com/contract' + token, body, {headers: headers})
          .map((response: Response) => {
              const result = response.json();
          })
          .catch((error: Response) => Observable.throw(error.json()));
    ...
    getContracts(type) {
      const token = localStorage.getItem('token')
        ...
        return this.http.get('https://warsha-2.herokuapp.com/contract' +  typeQuery)
            .map((response: Response) => {
    ...
    getContract(id) {
      const token = localStorage.getItem('token')
          ? '?token=' + localStorage.getItem('token')
          : '';
        return this.http.get('https://warsha-2.herokuapp.com/contract/' +id+  token)
            .map((response: Response) => {
    ...
    addFeedBack(id,feedback,type){
      ...
      return this.http.post('https://warsha-2.herokuapp.com/feedback/' + id + typeParam  ,feedback)
          .map((response: Response) => response.json())
          .catch((error: Response) => Observable.throw(error.json()));
    }
    getFeedback(id,status){
      const token = localStorage.getItem('token')
          ? '?token=' + localStorage.getItem('token')
          : '';
        return this.http.get('https://warsha-2.herokuapp.com/feedback/' + id + '/' +  status + token)
            .map((response: Response) => {
