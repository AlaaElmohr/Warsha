...
export class PostService{
    ...
    addPost(post:Post){
      ...
      const token = localStorage.getItem('token')
          ...
          : '';
      return this.http.post('http://app-warsha-1.herokuapp.com/post' + token, body, {headers: headers})
    ...
    updatePost(post,id){
      ...
      const token = localStorage.getItem('token')
          ...
          : '';
          console.log(id);
      return this.http.patch('http://app-warsha-1.herokuapp.com/post/' + id + token, body, {headers: headers})
          .map((response: Response) => {
              const result = response.json();
          })
          .catch((error: Response) => Observable.throw(error.json()));
