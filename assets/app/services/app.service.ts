...
export class AppService{
    ...
    addApplication(app:Application){
      ...
      const token = localStorage.getItem('token')
          ...
          : '';
      return this.http.post('http://app-warsha.herokuapp.com/application' + token, body, {headers: headers})
    ...
    getApps(id,type) {
      const token = localStorage.getItem('token')
          ? '?token=' + localStorage.getItem('token')
          : '';
        return this.http.get('http://app-warsha.herokuapp.com/application/' + id + '?query=' + type )
            .map((response: Response) => {
