...
changePassword(password){
  ...
  return this.http.patch('http://app-warsha-1.herokuapp.com/user/changepassword/' + id +token, body)
...
const query='?query=' + body +'&' +token;
  return this.http.get('http://app-warsha-1.herokuapp.com/user' + query)
...
getUser(value,type){
  const token = localStorage.getItem('token')
   ...
   let result;
  result= this.http.get('http://app-warsha-1.herokuapp.com/user/'+value+ query);
