...
changePassword(password){
  ...
  return this.http.patch('https://warsha-2.herokuapp.com/user/changepassword/' + id +token, body)
...
const query='?query=' + body +'&' +token;
  return this.http.get('https://warsha-2.herokuapp.com/user' + query)
...
getUser(value,type){
  const token = localStorage.getItem('token')
   ...
   let result;
  result= this.http.get('https://warsha-2.herokuapp.com/user/'+value+ query);
